...
handleClick(event){
 ...
 .then(function (response) {
   ...
   if(response.data.code === 200){
     ...
     uploadScreen.push(<UploadScreen appContext={self.props.appContext} userEmail={response.data.email} first={response.data.first}
                        last={response.data.last} picSource={iSource} bio={response.data.bio} admin={response.data.admin}/>)
